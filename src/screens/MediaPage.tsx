...
import { mergeVideos, getTmpOutputVideo } from '../utils';
...
export function MediaPage({ navigation }: Props): React.ReactElement {
  ...
  const outputVideoPath = useMemo(() => getTmpOutputVideo('mov'), []);

  useFocusEffect(
    React.useCallback(() => {
      mergeVideos(videos, outputVideoPath)
        .then(() => {
          setIsReady(true);
        })
        .catch((err) => console.log(err));
    }, [videos, outputVideoPath]),
  );
  ...
  const onSavePressed = useCallback(async () => {
    ...
    try {
      ...
      await CameraRoll.save(`file://${outputVideoPath}`, { type });
  ...
  if (!isReady || videos.length === 0) return <View />;

  return (
    <View style={[styles.container, screenStyle]}>
      <Video
        source={{ uri: outputVideoPath }}
