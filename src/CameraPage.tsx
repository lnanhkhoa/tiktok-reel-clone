...
import { CAPTURE_BUTTON_SIZE, CONTENT_SPACING, MAX_ZOOM_FACTOR, SAFE_AREA_PADDING, SCREEN_WIDTH, HEADER_HEIGHT } from './Constants';
...
const SPEED = [0.5, 1, 2, 3];
const STANDARD_VIDEO_LENGTH = [15, 30, 60];
...
export function CameraPage({ navigation }: Props): React.ReactElement {
  ...
  const canChooseSpeedMode = true;
  const canChooseStandardVideoLength = true;
  ...
  const onMediaCaptured = useCallback((media: PhotoFile | VideoFile, type: 'photo' | 'video') => {
    ...
    if (type === 'video') videoDispatch({ type: 'addVideo', payload: media });
  ...
  return (
    <View style={styles.container}>
      ...
      <PressableOpacity style={styles.closeButton} onPress={() => {}}>
          ...
          <PressableOpacity
            ...
            onPress={() =>
              setSpeed((value) => {
                ...
                return SPEED[(index + 1) % SPEED.length];
              })
            }
            disabledOpacity={0.4}>
      ...
      <View style={styles.bottomRightButtons}>
        <PressableOpacity onPress={() => {}} disabledOpacity={0.4}>
          <MaterialIcon name="minus-box" size={BUTTON_SIZE} color={'white'} />
        </PressableOpacity>
        <PressableOpacity style={styles.checkmarkWrapper} onPress={() => {}} disabledOpacity={0.4}>
